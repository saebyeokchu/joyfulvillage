            ...
            <div className="flex flex-col space-y-3">
                <div className="grid grid-cols-3 gap-3">
                    { images.length > 0 && images.map((src : string, index : number) => 
                        <div className="border relative" key={`manage_cafe_mainimg_${index}`}>
                            ...
                            <p className="w-full flex cursor-pointer absolute bottom-0" >
                                <span className="mx-auto h-8 pt-2 text-xs bg-green w-full text-center  text-white" onClick={()=>onClickDeleteImage(src)}>삭제하기</span>
                            </p>
                        </div>
                    )}
