...
export default function LevelOneSokso({
    ...
    const updateSoksos = async () => {
        await soksoService.getSoksoByLevel(1).then((response : any)=>{
            setSoksos(response);
        });
    }
    ...
    return(
        <>
            ...
            <div className="flex flex-col space-y-3  mt-3">
                {soksos.filter(e=>e.level==1 ).map((sokso : Sokso)=>
                    <div key={`level-one-${sokso.id}`} className="w-2/3 border-0 border-0-gray-200 rounded-lg hover:shadow-sm focus:outline-none " >
                        <div className="relative flex items-center overflow-hidden ">
                            ...
                            <div className="grow p-4 ms-32 sm:ms-48"> 
                                ...
                                <div className="flex flex-row space-x-3">
                                    ...
                                    <EditButton onClickFunction={() => onClickEditSokso(sokso)} btnName={"숙소 대표 내용 수정하기"} />
            ...
            { showUpsertSokso && <UpsertSokso level={1} targetSokso={editTargetSokso} onCloseModal={() => setShowUpsertSokso(false)} updateSoksos={updateSoksos} group={null}/> }
            {/* { showEditSokso && <EditSokso targetSokso={editTargetSokso} close={()=>setshowEditSokso(false)}/> } */}
        </>
    )
}