...
import { GetCafeDataBySection, ReplaceImg, UpdateCafeData } from "@/lib/url";
...
export default function ManageProgram(){
    ...
    const { data, error, mutate  } = useSWR<Cafe[]>(
        GetCafeDataBySection + CafeSection.tea,
        getFetcher
    );
    ...
    return (
        <>
        <AdminWrapper>
           <CafeWrapper subTitle={"티 메뉴 관리하기"} buttons={[{onClickFunction :onClickSaveCoffee, btnName : "저장하기"}]}>
                {undefined}
           </CafeWrapper>
