import { MegaMenu, Menu } from "../types/Types";

const BackgroundColor = '#F1EBDD';

const NaverBookingLink = 'https://map.naver.com/p/entry/place/1024078581?placePath=%252Fhome%253Fentry%253Dplt&searchType=place&lng=129.3545276&lat=36.3070483&c=15.00,0,0,0,dh';
const NaverBlogTweleveMountLink = 'https://blog.naver.com/navhayoungin';
const InstagramHayoungingLink = 'https://www.instagram.com/hayoungin7/?hl=en';
const InstagramJoyfulLink = 'https://www.instagram.com/joyvil_company/';
const InstagramSoopStayLink = 'https://www.instagram.com/forest_stay_docheon/?hl=en';
const InstgramCafeDocehnLink = 'https://www.instagram.com/cafe.docheon/';

const AdminCode = 'logginedasadmin';

const AdminEmailAddress = 'dbal6436@naver.com'; //dbal6436@naver.com

const AdminApiAddress = 'http://localhost:8000/api/v1/joyfulset';
const BaseApiAddress = 'http://localhost:8000/api/v1';

const ApiAddress = 'http://localhost:8000/api/v1';

const ResetCode = "FeMOUu";
const ManagerName = "유미";

const HomeSpaceCafeImgSrc = "/system/home/spacecafe.jpg";
const HomeSpaceBookImgSrc = "/system/home/spacebook.jpg";
const HomeSpaceSoopImgSrc = "/system/home/spacesoop.jpg";

const NoImgSrc = "/images/system/no-image.png";

const StringDivider = ";";

export const header_menus : Menu[] = [
    {
        title : "홈",
        iconSvg : <svg className="shrink-0 size-4 me-3 md:me-2 block md:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
    },{
        title : "숙소",
        iconSvg : <svg className="shrink-0 size-4 me-3 md:me-2 block md:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 12h.01"/><path d="M16 6V4a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/><path d="M22 13a18.15 18.15 0 0 1-20 0"/><rect width="20" height="14" x="2" y="6" rx="2"/></svg>
    },{
        title : "프로그램",
        iconSvg : <svg className="shrink-0 size-4 me-3 md:me-2 block md:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 22h16a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2H8a2 2 0 0 0-2 2v16a2 2 0 0 1-2 2Zm0 0a2 2 0 0 1-2-2v-9c0-1.1.9-2 2-2h2"/><path d="M18 14h-8"/><path d="M15 18h-5"/><path d="M10 6h8v4h-8V6Z"/></svg>
    },{
        title : "실시간 예약",
        iconSvg :  <svg className="shrink-0 size-4 me-3 md:me-2 block md:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 22h16a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2H8a2 2 0 0 0-2 2v16a2 2 0 0 1-2 2Zm0 0a2 2 0 0 1-2-2v-9c0-1.1.9-2 2-2h2"/><path d="M18 14h-8"/><path d="M15 18h-5"/><path d="M10 6h8v4h-8V6Z"/></svg>
    },{
        title : "카페",
        iconSvg :  <svg className="shrink-0 size-4 me-3 md:me-2 block md:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 22h16a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2H8a2 2 0 0 0-2 2v16a2 2 0 0 1-2 2Zm0 0a2 2 0 0 1-2-2v-9c0-1.1.9-2 2-2h2"/><path d="M18 14h-8"/><path d="M15 18h-5"/><path d="M10 6h8v4h-8V6Z"/></svg>
    },{
        title  : "문의하기",
        iconSvg :  <svg className="shrink-0 size-4 me-3 md:me-2 block md:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 22h16a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2H8a2 2 0 0 0-2 2v16a2 2 0 0 1-2 2Zm0 0a2 2 0 0 1-2-2v-9c0-1.1.9-2 2-2h2"/><path d="M18 14h-8"/><path d="M15 18h-5"/><path d="M10 6h8v4h-8V6Z"/></svg>
    }
]

export const header_mega_menus : MegaMenu[] = [
    {
        disabled : false,
        mainTitle : "홈",
        mainTitleRoute : "/",
        // iconSvg : <svg className="shrink-0 size-4 me-3 md:me-2 block md:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
    },{
        disabled : false,
        mainTitle : "숙소",
        mainTitleRoute : "/sokso",
        // iconSvg : <svg className="shrink-0 size-4 me-3 md:me-2 block md:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 12h.01"/><path d="M16 6V4a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/><path d="M22 13a18.15 18.15 0 0 1-20 0"/><rect width="20" height="14" x="2" y="6" rx="2"/></svg>,
        // subTitle : [
        //     {
        //         name : "숙소 전체 보기",
        //         route : "/lodging"
        //     },
        //     {
        //         name : "숲 스테이 도천",
        //         route : "/lodging/soop"
        //     },{
        //         name : "북 스테이 도천",
        //         route : "/lodging/book"
        //     }
        // ]
    },
    {
        disabled : false,
        mainTitle : "프로그램",
        mainTitleRoute : "/program",
        // iconSvg : <svg className="shrink-0 size-4 me-3 md:me-2 block md:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 22h16a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2H8a2 2 0 0 0-2 2v16a2 2 0 0 1-2 2Zm0 0a2 2 0 0 1-2-2v-9c0-1.1.9-2 2-2h2"/><path d="M18 14h-8"/><path d="M15 18h-5"/><path d="M10 6h8v4h-8V6Z"/></svg>
    },
    {
        disabled : true,
        mainTitle : "실시간예약",
        mainTitleRoute : "/booking",
        // iconSvg :  <svg className="shrink-0 size-4 me-3 md:me-2 block md:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 22h16a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2H8a2 2 0 0 0-2 2v16a2 2 0 0 1-2 2Zm0 0a2 2 0 0 1-2-2v-9c0-1.1.9-2 2-2h2"/><path d="M18 14h-8"/><path d="M15 18h-5"/><path d="M10 6h8v4h-8V6Z"/></svg>
    },
    {
        disabled : false,
        mainTitle : "카페",
        mainTitleRoute : "/cafe",
        // iconSvg :  <svg className="shrink-0 size-4 me-3 md:me-2 block md:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 22h16a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2H8a2 2 0 0 0-2 2v16a2 2 0 0 1-2 2Zm0 0a2 2 0 0 1-2-2v-9c0-1.1.9-2 2-2h2"/><path d="M18 14h-8"/><path d="M15 18h-5"/><path d="M10 6h8v4h-8V6Z"/></svg>
    },
    {
        disabled : false,
        mainTitle : "문의하기",
        mainTitleRoute : "/inquiry",
        // iconSvg :  <svg className="shrink-0 size-4 me-3 md:me-2 block md:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 22h16a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2H8a2 2 0 0 0-2 2v16a2 2 0 0 1-2 2Zm0 0a2 2 0 0 1-2-2v-9c0-1.1.9-2 2-2h2"/><path d="M18 14h-8"/><path d="M15 18h-5"/><path d="M10 6h8v4h-8V6Z"/></svg>
    }
]

const mobilePx = "px-4 sm:px-6 lg:px-8";

export {
    BackgroundColor,

    NaverBookingLink,
    NaverBlogTweleveMountLink,
    InstagramHayoungingLink,
    InstagramJoyfulLink,
    InstagramSoopStayLink,
    InstgramCafeDocehnLink,

    AdminCode,
    AdminApiAddress,
    AdminEmailAddress,
    BaseApiAddress,
    ApiAddress,
    ResetCode,
    ManagerName,
    HomeSpaceCafeImgSrc,
    HomeSpaceBookImgSrc,
    HomeSpaceSoopImgSrc,
    NoImgSrc,
    StringDivider,
    mobilePx
}
